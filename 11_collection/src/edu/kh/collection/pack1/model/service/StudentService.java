...
public class StudentService {
	...
	// * 인처페이스 객체 생성 X, 부모 참조변수 O
	
	// java.util.ArrayList : 배열 형태 List (가장 대표적인 List 자식 클래스)
	
	// new ArrayList() : 기본 생성자 -> 기본 크기가 10짜리 리스트 생성
	//					 -> 하지만 리스트는 크기가 늘었다 줄었따 하기 때문에 큰 의미 없음
	...
	*/
	...
	public String removeStudent() throws InputMismatchException{
		...
		// remove() 동작 시 뒤쪽 요소를 한칸씩 알아서(자동으로) 당겨온다
		...
		// 1) 학생 정보가 studentList에 있는지 검사 : 한명도 없는지 검사
				...
				// 4) 만약 문자열을 입력한 경우 -> 예외처리 throws
					...
					// String.toLowerCase() : 문자열을 소만자로 변경
					
					if(ch == 'Y') { // 삭제
						Student temp = studentList.remove(index);
						return temp.getName() + "정보가 제거되었습니다";
					}else {
						return "취소";
					}
				}
	 ...
	 * 검색할 이름 입력받아 studentList에서 꺼내온 Sutedent 객체의 name 값이 같으니 비교
	...
	public void searchName2() {
		...
		for(Student std : studentList) {
			...
			// : String에 문자열이 포함되어 있으면 true / 없으면 false
	...
	public void sortByName() {
		...
		// Student::getName -> 메서드 레퍼런스(Meㅅhod Referance)
