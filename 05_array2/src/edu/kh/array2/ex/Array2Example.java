...
public class Array2Example {
	...
	public void ex2() {
		...
		// 1행의 합 : 15
		// 2행의 합 : 24
		for(int y = 0; y < arr.length; y++) { // 행 반복
			
			int sum = 0;
			
			for(int x = 0; x<arr[y].length; x++) { // 열 반복
		...
		for(int x = 0; x <arr[0].length; x++) { // 각 열을 순회

			int sum = 0; // 현재 열의 합을 저장할 변수
			
			for(int y = 0; y < arr.length; y++) { // 각 행을 순회
				sum += arr[y][x];
			}
	...
	public void ex3() {
		...
		arr[1] = new char[4]; // 0행에 3열짜리 1차원 배열을 생성하여 주소값 저장
		arr[2] = new char[5]; // 0행에 3열짜리 1차원 배열을 생성하여 주소값 저장
		arr[3] = new char[2]; // 0행에 3열짜리 1차원 배열을 생성하여 주소값 저장
