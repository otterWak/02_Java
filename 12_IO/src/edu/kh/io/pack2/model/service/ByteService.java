...
public class ByteService {
	 ...
	 * 
	 * - 최상위 클래스 : InputStram, OutputStream
	 * 
	...
	public void fileByteOutput() {
		...
		// FileOutStream 참조 변수 선언 (OutputStream을 상속 받은 자식 클래스)
		...
		try {
			...
			// StringBilder(비동기) / StringBuffer(동기)
	...
	public void bufferedFileByteOutput() {
		...
		/* 버퍼(Buffer) 란?
		 * - 데이터를 모아두는 박스, 바구니 같은 메로리 공간
		 * 
		 * * 버퍼링
		 * - 인터넷 속도 낮음 + 스트리밍 고드가 효율이 안좋음
		 * -> 동영상 재생 중에 미리 앞에 재생될 부분을 다운 받아놔서
		 * 	  영상을 끊김없이 볼 수 있게함
		 * -> 다운 받아서 저장해놓는 공간 == 버퍼
		 * 
		 *    BufferedOutputStream    /    BufferedInputStream
		 * 출력용 보조 스트림(바이트) / 입력용 보소 스트림(바이트)
		 * 
		...
		FileOutputStream fos = null; //O기반 스트림 참조변수
	...
	public void fileByteInput() {
		...
		try {
			...
			// IntputStream 만들 때는 해당 경로에 실제 파일이 있어야 함
			
			// 방법 1. 파일 내부 내용을 1byte씩 끊어서 가여조이
	 ...
	 * 
	 * InputStream의 경우 외부 파일을 읽어올 떄 사용
	 * -> 읽어오는 파일의 크기가 작으면 보조 스트림의 성능 이점이 크게 눈에띄지 않음
	 * 
	 */
	public void bufferedFileByteInpu() {
		// 파일 입력용 바이트 기반 스트림 선언
		FileInputStream fis = null;
	 ...
	 * 
	 * 3) 읽어온 내용을 가틍ㄴ 경로 위치에 "파일명_copy" 이름으로 출력
	 * 	  : FileOutputStream + BufferedOutptSteram
	 * 
